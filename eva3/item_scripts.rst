...
* all variables from the controller var file (:ref:`uc_cvars<eva3_uc_cvars>` or
  :ref:`lm_cvars<eva3_lm_cvars>`)
...
Item actions are used to control the units. After the :ref:`unit<eva3_unit>`
action has been called, the controller executes the appropriate script. By
default, control scripts are placed in xc/uc/ folder and named ID, where ID is
a unit ID, for example, xc/uc/lamp1. This may be changed in the item
configuration to let e.g. one script execute actions for a group of units.
...
from :ref:`uc_cvars<eva3_uc_cvars>`
...
where in :ref:`uc_cvars<eva3_uc_cvars>`:
...
In the :ref:`queue<eva3_uc_queues>` history script is marked as completed if it
...
:ref:`result<eva3_ucapi_result>` dict.
...
* After the :ref:`unit<eva3_unit>` action succeeds (if
...
:ref:`Multiupdates<eva3_multiupdate>` allow updating the state of several items
with one script which works like a normal passive update script and outputs the
...
tools<cli>`, with SYS API function :ref:`cmd<eva3_cmd>` or with :ref:`macro
function<eva3_macro_api_cmd>`.
