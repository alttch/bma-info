...
To execute a macro, use **macro run** command of :doc:`eva lm<../cli>` or LM
API :ref:`run<eva3_lmapi_run>` function.
...
appended to the controller using :ref:`create_macro<eva3_lmapi_create_macro>`
LM API function or with **eva lm**.

After the macro configuration is created, you may view its params using
:ref:`list_macro_props<eva3_lmapi_list_macro_props>` and change them with
:ref:`set_macro_prop<eva3_lmapi_set_macro_prop>`.
...
execution of other macros, use macro :ref:`lock<eva3_macro_api_lock>` and
:ref:`unlock<eva3_macro_api_unlock>` functions.
...
:ref:`logic variables<eva3_lvar>` timers.
...
than :ref:`logic variables<eva3_lvar>` or other loadable items update their
status (e. g. due to slow connection with :ref:`MQTT server<eva3_mqtt_>`) it's
recommended to use :ref:`sleep<eva3_macro_api_sleep>` function to do a small
delay.
...
set/reset required :ref:`logic variables<eva3_lvar>`. The macro should end its
work in default controller timeout.
...
:ref:`run<eva3_macro_api_run>` function, these macros will be executed even if
the API key allows to run only the initial macro.
...
* all :ref:`lm_cvars<eva3_lm_cvars>` variables
...
extensions<ext>`. As soon as extension is loaded, its functions become
available in all macros without a need to restart :doc:`LM PLC<lm>`.
...
.. _eva3_macro_api_decrement_shared:
...
.. _eva3_macro_api_increment_shared:
...
.. _eva3_macro_api_undeploy_device:
...
.. _eva3_macro_api_get_cycle_info:
...
.. _eva3_macro_api_is_cycle_running:
...
.. _eva3_macro_api_list_cycle_props:
...
.. _eva3_macro_api_reset_cycle_stats:
...
.. _eva3_macro_api_set_cycle_prop:
