...
:ref:`logic variable (lvar)<eva3_lvar>`. Logic variables are used to save and
exchange system status data as well as function as flags and timers for
organizing production cycles.

Any change in the status of :ref:`units<eva3_unit>`,
:ref:`sensors<eva3_sensor>` or logic variables is analyzed by the
:doc:`decision-making matrix<decision_matrix>` of
...
connected to :ref:`MQTT server<eva3_mqtt_>`.
...
.. literalinclude:: ../cfg/lm_main.yml
    :language: yaml
...
Manager loads information about :ref:`units<eva3_unit>` and
:ref:`sensors<eva3_sensor>` from the connected :doc:`Universal
Controllers<../uc/uc>`.

Prior to UC connection, it is necessary to connect LM PLC to :ref:`MQTT
server<eva3_mqtt_>`, where other controllers will send the events. Logic
Manager reads the list of items and their initial status from the connected
...
To control the list of the received items you can use *eva lm* or :doc:`lm_api`
function :ref:`list_remote<eva3_lmapi_list_remote>`:
...
Parameters are displayed with **eva lm** command or :doc:`lm`
:ref:`list_controller_props<eva3_lmapi_list_controller_props>` function,
modified with :ref:`set_controller_prop<eva3_lmapi_set_controller_prop>`.
Function :ref:`list_controllers<eva3_lmapi_list_controllers>` displays the list
of all connected controllers.

To remove the connected controller use
:ref:`remove_controller<eva3_lmapi_remove_controller>` function.
...
ones, one should use :ref:`lock<eva3_macro_api_lock>` and
:ref:`unlock<eva3_macro_api_unlock>` macro functions operating similarly to
:ref:`SYS API locking<eva3_sysapi_lock>`.

The status of the macro in queue is similar to the status of the :ref:`Universal
Controller actions<eva3_uc_queues>`.
