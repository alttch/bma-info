...
* Then, the system is verifying whether the current item state matches the rule
  conditions and the previous one is out of its range
    ...
    * :ref:`Logic variables<eva3_lvar>` always have an initial value stored in
      the local base, that's why **for_initial** should always be any for them
      to let the rules work correctly, unless you really know what you do.
    * If the rule matches, a :doc:`macro<macros>` (if specified) with the
      specified arguments is executed.
    * If chill-out_time > 0 in the configuration, the rule is ignored after the
...
Rules can be created with either LM API
:ref:`create_rule<eva3_lmapi_create_rule>` function or with :doc:`EVA
shell<../cli>`.
...
.. note::
    ...
    :ref:`set_rule_prop<eva3_lmapi_set_rule_prop>`, setting *enabled=True*.
...
* **for_prop** the state property of the item (**status** or **value**) the
  rule is checking. For :ref:`unit<eva3_unit>` state, **nstatus** and
  **nvalue** properties may be additionally used.
...
* if you set a field **for_expire** (with any value, i.e. *Y*) in a rule change
  ...
  useful to configure the rule to check for the :ref:`lvar<eva3_lvar>` timers
  expiration as well as checking for :ref:`units<eva3_unit>` and
  :ref:`sensors<eva3_sensor>` error states
...
* if you set a field **for_set** (with any value, i.e.  *Y*) in a rule change
  ...
  configure the rule to check for the :ref:`lvar<eva3_lvar>` timers reset as
  well as working with a logical flags
