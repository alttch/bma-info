...
* **Event subject** (not to be confused with :ref:`MQTT<eva3_mqtt_>` subject) *
...
Notification sends data similar to those, one can get using :doc:`uc/uc_api` or
:doc:`lm/lm_api` state.  There is one difference for a
:ref:`sensors:<eva3_sensor>`: sensor with error status (status = -1) does not
send its value data until the value is null. This was done specifically for the
...
Every time the :ref:`unit<eva3_unit>` :ref:`action<eva3_ucapi_action>` or
:doc:`macro action<lm/macros>` changes its :ref:`status<eva3_uc_queues>`, the
notification
server receives "action" event notification.

Notification sends data similar to ones that can be obtained using UC API
:ref:`result<eva3_ucapi_result>` command.
...
* **collect_logs** this should be set to "true" for :doc:`sfa/sfa`
  :ref:`MQTT<eva3_mqtt_>` notifiers if you want to collect the logs of other
...
:ref:`queued actions:<eva3_uc_queues>`.
...
:ref:`Unit<eva3_unit>` action notifications are sent to the topic
...
You may specify different :ref:`MQTT<eva3_mqtt_>` QoS for events with different
subjects.
...
MQTT can be also used as API to send actions to the :ref:`units<eva3_unit>`. In
...
To send events in :ref:`bulk<eva3_bulk_notify>`:
...
networks. Actually, Using LURP is equal of using :ref:`MQTT
notifiers<eva3_mqtt_>` with QoS 0, but without the central data exchange point
and with more lightweight UDP packets.
...
config<eva3_lm_config>` or :ref:`SFA config<eva3_sfa_config>`.

If :ref:`bulk notifications<eva3_bulk_notify>` are used, make sure LURP buffer
...
:doc:`/eva-js-framework/index` for analysis and e.g. to build graphical charts.
...
After, you can tell :ref:`state_history <eva3_sfapi_state_history>` SFA API
function to select metrics from *daily* retention policy, specifying additional
...
To use bulk notifications with :ref:`MQTT<eva3_mqtt_bulk>`, additional options
need to be set.

To use bulk notifications with web sockets (between controllers and with
:doc:`EVA JS Framework </eva-js-framework/index>`, *ws_buf_ttl* option need to
be set in the controller props (on receivers), or in case of
framework, with
...
:ref:`list_notifiers<eva3_sysapi_list_notifiers>` API method can be used.
...
To switch between :ref:`MQTT<eva3_mqtt_>` and :ref:`PSRT<eva3_psrt_>`
on-the-flow, use the following command:
