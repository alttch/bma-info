...
It aggregates all the :doc:`control and monitoring items<../items>`,
:doc:`logic macros<../lm/macros>` and :doc:`decision
rules<../lm/decision_matrix>` from all connected :doc:`UC<../uc/uc>` and
:doc:`LM PLC<../lm/lm>` controllers into one place. As a result, the final
interface or application doesn't need to know on which controller the
:doc:`item<../items>` is present, it does all the function calls directly to
SFA. Ids of the items should be always specified in oid form (*type:group/id*)
...
SFA is set up and controlled with **eva sfa** :doc:`console
application<../cli>` and :doc:`sfa_api`. The API doesn't have a user interface
by default, it's developed specifically for certain installation certain
installation using :doc:`sfa_templates` (server-side part) and
:doc:`/eva-js-framework/index` (client-side part).
...
notification aggregator e. g. by transferring :ref:`MQTT<eva3_mqtt_>` messages
to other application via HTTP or redirecting them to other MQTT servers. SFA
...
.. literalinclude:: ../cfg/sfa_main.yml
    :language: yaml
...
:doc:`/eva-js-framework/index` after login with any valid user or API key.
...
connecting :doc:`UC<../uc/uc>` and :doc:`LM PLC<../lm/lm>` remote controllers,
it is necessary to connect SFA to :ref:`MQTT<eva3_mqtt_>` server (via its
notification system), to which other controllers will send the events. SFA
reads the list of :doc:`items<../items>`, :doc:`macros<../lm/macros>`,
...
command<../cli>` or SFA API
:ref:`append_controller<eva3_sfapi_append_controller>` function.
...
:doc:`sfa_api` function :ref:`list_remote<eva3_sfapi_list_remote>`:
...
* **mqtt_update** :ref:`MQTT notifier<eva3_mqtt_>` through which items update
  their status
...
Parameters are displayed with **eva sfa** command or :doc:`sfa`
:ref:`list_controller_props<eva3_sfapi_list_controller_props>` function,
modified with :ref:`set_controller_prop<eva3_sfapi_set_controller_prop>`.
Function :ref:`list_controllers<eva3_sfapi_list_controllers>` displays the list
of all connected controllers.

To remove the connected controller use
:ref:`remove_controller<eva3_sfapi_remove_controller>` function.
...
:doc:`sfa_templates`, :doc:`/eva-js-framework/index` and :doc:`sfa_pvt`.
Interface files are stored in **ui** folder, interface is available at
